...
export class TipologiaVeicoloComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.tipologiaVeicoloService
      ...
      .subscribe(
        (res: HttpResponse<ITipologiaVeicolo[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ITipologiaVeicolo): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInTipologiaVeicolos(): void {
    this.eventSubscriber = this.eventManager.subscribe('tipologiaVeicoloListModification', () => this.loadPage());
  }

  delete(tipologiaVeicolo: ITipologiaVeicolo): void {
    const modalRef = this.modalService.open(TipologiaVeicoloDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.tipologiaVeicolo = tipologiaVeicolo;
  }
  ...
  protected onSuccess(data: ITipologiaVeicolo[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
